...
int				is_redirection(t_token **token, t_cmd **command,
					t_token **last_tkn);
...
t_token			*expansor(t_token *head, t_env **env, int is_hdoc,
					int is_quoted);
...
char			*construct_full_path(const char *dir_path,
...
t_cmd			*process_command_block(t_cmd *cmd, int *err_code,
					t_env *tenv, int pos);
int				is_last_cmd_in_pipe(t_cmd *cmd, int pos);
void			update_pipes(t_pipe *fds, int j, t_cmd *next_cmd);
int				process_child(t_cmd *cmd, t_pipe *fds, t_env *tenv,
					int cmd_index);
...
int				process_hdoc_child(char *word, t_pipe *fds, int expand,
					t_env **tenv);
...
/* SIGNAL */
void			handle_update_signal(t_signal *signals,
					enum e_signal_handle mode);
